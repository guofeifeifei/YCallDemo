...
-(void)showLineMenu{
    ...
    for (int  i = 1; i < menuButtonArray.count + 1; i++) {
        
       
        [UIView animateWithDuration:ANIMATION_DURATION delay:0 options:UIViewAnimationOptionCurveEaseInOut animations:^{
            pMenuButtonSuperView.layer.opacity = 1.0;
            UIButton * menuButton = (UIButton *)[menuButtonArray objectAtIndex:i- 1];
            menuButton.layer.opacity = 1.0;
            menuButton.center = CGPointMake(self.menuButton.center.x, self.menuButton.center.y -60 * i);
        }completion:nil];
    }
}
...
-(void)showHorLineMenu{
    ...
    for (int  i = 1; i < menuButtonArray.count + 1; i++) {
        [UIView animateWithDuration:ANIMATION_DURATION delay:0 options:UIViewAnimationOptionCurveEaseInOut animations:^{
            ...
            UIButton * menuButton = (UIButton *)[menuButtonArray objectAtIndex:i- 1];
            menuButton.layer.opacity = 1.0;
            menuButton.center = CGPointMake(self.menuButton.center.x + 60 * i, self.menuButton.center.y );
        }completion:nil];
    }
}
-(void)hideHorLineMenu{
     [self hideCircularMenu];
}
