...
NS_INLINE MBProgressHUD *settHUD(UIView *view, NSString *title, BOOL autoHidden) {
    ...
    hud.label.font = [UIFont systemFontOfSize:10 * kwidthScale];
    //支持多行
...
+ (void)showTitleToViewSet:(UIView *)view
                     title:(NSString *)title{
    MBProgressHUD *hud = settHUD(view, title, YES);
    hud.mode = MBProgressHUDModeText;
    hud.offset = CGPointMake(0.f, KMainScreenHeight/2 - 130* kwidthScale);
  
}
+ (MBProgressHUD *)showLoadToViewSet:(UIView *)view{
    MBProgressHUD *hud = settHUD(view, @"加载中", NO);
    hud.mode = MBProgressHUDModeIndeterminate;
    hud.offset = CGPointMake(0.f, -KMainScreenHeight/2);
